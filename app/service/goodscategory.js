...
class GoodsCategoryService extends Service {
  ...
  async findAllTopCates() {
    try {
      var topCates = await this.ctx.model.GoodsCategory.find({ cate_pid: "0",data_status:1 });
  ...
  async delete(_id) {
    try {
      ...
      if(catePid == '0'){
        ...
        var a = await this.ctx.model.GoodsCategory.updateMany({ cate_pid: _id },{data_status:0});
       var b = await this.ctx.model.GoodsCategory.updateOne({ _id: _id },{data_status:0});
       console.log(JSON.stringify(a)+'xxxxx'+JSON.stringify(b));
       

      }else{
        await this.ctx.model.GoodsCategory.updateOne({ _id: _id },{data_status:0});
      }
